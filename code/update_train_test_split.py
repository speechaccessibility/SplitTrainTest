...
Revision History: 
Original release: 2023 July 17
Modified cost to be average Wasserstein rather than sum: 2023 July 18
...
class Score_Cache():
    ...
    def score_assignment(self, assignment):
        ...
        Score = average over pairs (train,dev), (train,test), (test,dev)
          of average over feature dimensions
        ...
        for each feature dimension:
         ...
         score = average of absolute differences
        ...
        for fkey,fval in self.feature.items():
            ...
            for (s0,s1) in [(tr,de),(tr,te),(te,de)]:
                if len(s1) > len(s0):  # Make sure s0 is longer
                    s0,s1 = s1,s0
                if len(s0)>0:
                    denominator = 3*len(s0)*len(self.feature) # so that result is an average, not a sum
                    if len(s1) == 0:  # if len(s1)==0, put its probability mass at 0
                        score += np.sum(s0)/denominator
                    else:             # prob. mass quantized in units of 1/len(s0)
                        i0 = np.arange(len(s0))
    ...
    def neighbor_search(self, original):
        ...
        @return:
        score (real): score of best assignment (average of L1 Wasserstein distances)
    ...
    def create_output_dict(self, assignment, score):
        ...
        score (real) - average across pairs of average across features of the L1 Wasserstein distance.
